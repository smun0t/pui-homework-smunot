...
function cartAdd() {
    ...
    // Get the notification span element
    const notificationSpan = document.querySelector('.notification');

    // Update the content of the notification span
    notificationSpan.textContent = cart.length.toString();
...
// Function to save cart to JSON and store it in local storage
...
function getCartLocal() {
    ...
    console.log(cartListString);
    if (cartListString) {
        const cartArray = JSON.parse(cartListString);
        cartArray.forEach(rollData => {
            const cartItem = new Roll(rollData.type, rollData.glazing, rollData.size, rollData.basePrice);
            cart.push(cartItem);
...
// Check if cart exists in local storage, if yes, retrieve it, else, create an empty cart array
